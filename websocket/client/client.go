...
// New 新建一个连接对象
func (c *Client) New(session m_dto.Session) websocket.WebSocket {
	return &Client{
...
func (c *Client) Listening() error {
	...
	for {
		...
		case <-c.heartBeatTicker.C:
			...
			heartBeatEvent := &m_dto.WSPayload{
				WSPayloadBase: m_dto.WSPayloadBase{
					OPCode: m_dto.WSHeartbeat,
				},
...
func (c *Client) Write(message *m_dto.WSPayload) error {
	m, _ := json.Marshal(message)
	log.Infof("%s write %s message, %v", c.session, m_dto.OPMeans(message.OPCode), string(m))
...
func (c *Client) Resume() error {
	event := &m_dto.WSPayload{
		Data: &m_dto.WSResumeData{
			Token:     c.session.Token.GetString(),
			SessionID: c.session.ID,
			Seq:       c.session.LastSeq,
		},
	}
	event.OPCode = m_dto.WSResume // 内嵌结构体字段，单独赋值
...
func (c *Client) Identify() error {
	...
	if c.session.Intent == 0 {
		c.session.Intent = m_dto.IntentGuilds
	}
	event := &m_dto.WSPayload{
		Data: &m_dto.WSIdentityData{
			Token:   c.session.Token.GetString(),
...
func (c *Client) Session() *m_dto.Session {
	return c.session
}
...
func (c *Client) readMessageToQueue() {
	for {
		...
		log.Infof("%s receive %s message, %s", c.session, m_dto.OPMeans(event.OPCode), string(message))
...
func (c *Client) isHandleBuildIn(event *m_dto.WSPayload) bool {
	switch event.OPCode {
	case m_dto.WSHello: // 接收到 hello 后需要开始发心跳
		c.startHeartBeatTicker(event.RawMessage)
	case m_dto.WSHeartbeatAck: // 心跳 ack 不需要业务处理
	case m_dto.WSReconnect: // 达到连接时长，需要重新连接，此时可以通过 resume 续传原连接上的事件
		c.closeChan <- errs.ErrNeedReConnect
	case m_dto.WSInvalidSession: // 无效的 sessionLog，需要重新鉴权
...
func (c *Client) readyHandler(payload *m_dto.WSPayload) {
	readyData := &m_dto.WSReadyData{}
