...
type ChanManager struct {
	sessionChan chan m_dto.Session
}

// Start 启动本地 session manager
func (l *ChanManager) Start(apInfo *m_dto.WebsocketAP, token *m_token.MToken, intents *m_dto.Intent) error {
	defer log.Sync()
	...
	l.sessionChan = make(chan m_dto.Session, apInfo.Shards)
...
func (l *ChanManager) newConnect(session m_dto.Session) {
	defer func() {
