...
func (o *openAPI) ChannelPermissions(ctx context.Context, channelID, userID string) (*m_dto.ChannelPermissions, error) {
	rsp, err := o.request(ctx).
		SetResult(m_dto.ChannelPermissions{}).
	...
	return rsp.Result().(*m_dto.ChannelPermissions), nil
}

// ChannelRolesPermissions 获取指定子频道身份组的权限
func (o *openAPI) ChannelRolesPermissions(ctx context.Context,
	channelID, roleID string) (*m_dto.ChannelRolesPermissions, error) {
	rsp, err := o.request(ctx).
		SetResult(m_dto.ChannelRolesPermissions{}).
		SetPathParam("channel_id", channelID).
