// Package v1 是 m_openapi v1 版本的实现。
package v1
...
// Setup 注册
func Setup() {
	m_openapi.Register(m_openapi.APIv1, &openAPI{})
}

// Version 创建当前版本
func (o *openAPI) Version() m_openapi.APIVersion {
	return m_openapi.APIv1
}
...
// Setup 生成一个实例
func (o *openAPI) Setup(token *m_token.MToken, inSandbox bool) m_openapi.OpenAPI {
	api := &openAPI{
		token:   token,
...
func (o *openAPI) WithTimeout(duration time.Duration) m_openapi.OpenAPI {
	...
	o.restyClient = resty.New().
		...
		SetPreRequestHook(
			func(client *resty.Client, request *http.Request) error {
				...
				return m_openapi.DoReqFilterChains(request, nil)
			},
		).
		...
		OnAfterResponse(
			func(client *resty.Client, resp *resty.Response) error {
				...
				// 执行请求后过滤器
				if err := m_openapi.DoRespFilterChains(resp.Request.RawRequest, resp.RawResponse); err != nil {
					return err
				}
				traceID := resp.Header().Get(m_openapi.TraceIDKey)
				o.lastTraceID = traceID
				// 非成功含义的状态码，需要返回 error 供调用方识别
				if !m_openapi.IsSuccessStatus(resp.StatusCode()) {
...
func respInfo(resp *resty.Response) string {
	...
	return fmt.Sprintf(
		...
		resp.Request.Method,
		resp.Request.URL,
		resp.Header().Get(m_openapi.TraceIDKey),
		resp.Status(),
		resp.Time(),
		string(bodyJSON),
		string(resp.Body()),
